...
-(void)update_ShenFenXX
{
    ...
    //查询 身份信息
    /*
    if ([[FMDBManager shareManager]selectShenFenXXByShenFenZJ:self.shenFenZJ].count>0) {
        NSLog(@"存在！");
        
        //修改 身份信息
        [[FMDBManager shareManager]updateShenFenXXByShenFenZJ:self.shenFenZJ dict:parameter];
    } else {
        NSLog(@"不存在！");
        
        //添加 身份信息
        [[FMDBManager shareManager]addShenFenXXData:parameter];
    }
    
    
    
    //删除 身份信息
//    [[FMDBManager shareManager]deleteShenFenXXByShenFenZJ:self.shenFenZJ];
     */
