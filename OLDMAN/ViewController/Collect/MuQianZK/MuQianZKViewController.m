...
-(void)create_tongZhuPO_View
{
    ...
    //配偶健康状况 标题
    UILabel * tongZhuPO_Label=[ZCControl createLabelWithFrame:CGRectMake(15, 0, WIDTH-60, Title_text_font) Font:Title_text_font Text:@"3. 同住配偶健康状况"];
    [tongZhuPO_View addSubview:tongZhuPO_Label];
    tongZhuPO_Label.textColor=Title_text_color;
...
-(void)update_MuQianZK
{
    ...
    //查询 目前生活状况
    /*
    if ([[FMDBManager shareManager]selectMuQianZKByShenFenZJ:self.shenFenZJ].count>0) {
        NSLog(@"存在！");
        
        //修改 目前生活状况
        [[FMDBManager shareManager]updateMuQianZKByShenFenZJ:self.shenFenZJ dict:parameter];
    } else {
        NSLog(@"不存在！");
        
        //添加 目前生活状况
        [[FMDBManager shareManager]addMuQianZKData:parameter];
    }
    
    
    //删除 目前生活状况
    //[[FMDBManager shareManager]deleteMuQianZKByShenFenZJ:self.shenFenZJ];
     */
