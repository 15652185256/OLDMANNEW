...
-(void)update_XinXiCJ
{
    ...
    //查询 信息采集初步结果
    /*
    if ([[FMDBManager shareManager]selectXinXiCJByShenFenZJ:self.shenFenZJ].count>0) {
        NSLog(@"存在！");
        
        //修改 信息采集初步结果
        [[FMDBManager shareManager]updateXinXiCJByShenFenZJ:self.shenFenZJ dict:parameter];
    } else {
        NSLog(@"不存在！");
        
        //添加 信息采集初步结果
        [[FMDBManager shareManager]addXinXiCJData:parameter];
    }
    
    
    //删除 信息采集初步结果
    //[[FMDBManager shareManager]deleteXinXiCJByShenFenZJ:self.shenFenZJ];
     */
