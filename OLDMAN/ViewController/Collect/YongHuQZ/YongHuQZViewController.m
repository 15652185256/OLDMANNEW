...
-(void)update_YongHuQZ
{
    ...
    //NSLog(@"%@-%@-%@-%@-%@",[user objectForKey:doc_id],self.shenFenZJ,my_xinXi,my_tongZhi,my_tuPian);
    

    //查询 用户签字
    /*
    if ([[FMDBManager shareManager]selectYongHuQZByShenFenZJ:self.shenFenZJ].count>0) {
        NSLog(@"存在！");
        
        //修改 用户签字
        [[FMDBManager shareManager]updateYongHuQZByShenFenZJ:self.shenFenZJ dict:parameter];
    } else {
        NSLog(@"不存在！");
        
        //添加 用户签字
        [[FMDBManager shareManager]addYongHuQZData:parameter];
    }
    
    
    //删除 用户签字
//    [[FMDBManager shareManager]deleteYongHuQZByShenFenZJ:self.shenFenZJ];
     */
