...
-(void)update_YiQueZhenJB
{
    ...
    //查询 已确诊疾病
    /*
    if ([[FMDBManager shareManager]selectYiQueZhenJBByShenFenZJ:self.shenFenZJ].count>0) {
        NSLog(@"存在！");
        
        //修改 已确诊疾病
        [[FMDBManager shareManager]updateYiQueZhenJBByShenFenZJ:self.shenFenZJ dict:parameter];
    } else {
        NSLog(@"不存在！");
        
        //添加 已确诊疾病
        [[FMDBManager shareManager]addYiQueZhenJBData:parameter];
    }
    
    
    //删除 已确诊疾病
    //[[FMDBManager shareManager]deleteYiQueZhenJBByShenFenZJ:self.shenFenZJ];
     */
