...
-(void)update_BuChongPG
{
    ...
    //查询 补充评估信息
    /*
    if ([[FMDBManager shareManager]selectBuChongPGByShenFenZJ:self.shenFenZJ].count>0) {
        NSLog(@"存在！");
        
        //修改 补充评估信息
        [[FMDBManager shareManager]updateBuChongPGByShenFenZJ:self.shenFenZJ dict:parameter];
    } else {
        NSLog(@"不存在！");
        
        //添加 补充评估信息
        [[FMDBManager shareManager]addBuChongPGData:parameter];
    }
    
    
    //删除 补充评估信息
    //[[FMDBManager shareManager]deleteBuChongPGByShenFenZJ:self.shenFenZJ];
     */
