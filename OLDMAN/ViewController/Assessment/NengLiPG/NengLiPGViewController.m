...
-(void)update_NengLiPG
{
    ...
    //查询 能力评估结论
    /*
    if ([[FMDBManager shareManager]selectNengLiPGByShenFenZJ:self.shenFenZJ].count>0) {
        NSLog(@"存在！");
        
        //修改 能力评估结论
        [[FMDBManager shareManager]updateNengLiPGByShenFenZJ:self.shenFenZJ dict:parameter];
    } else {
        NSLog(@"不存在！");
        
        //添加 能力评估结论
        [[FMDBManager shareManager]addNengLiPGData:parameter];
    }
    
    
    //删除 能力评估结论
    //[[FMDBManager shareManager]deleteNengLiPGByShenFenZJ:self.shenFenZJ];
     */
...
-(void)panduan_QianMing
{
    ...
    if ([[user objectForKey:idenity] intValue]==1) {
        if ([PublicFunction isBlankString:my_caiJi_qianMing]) {
            ...
            [alertView show];
        } else if (WIDTH==320  && my_caiJi_qianMing.length<2200) {
            ...
            [alertView show];
        } else if (WIDTH==375  && my_caiJi_qianMing.length<3000) {
            
            UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请正确填写“采集评估员签名”" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
            [alertView show];
        } else if (WIDTH==414  && my_caiJi_qianMing.length<4000) {
        ...
        if ([PublicFunction isBlankString:my_zhuZe_qianMing]) {
            ...
            [alertView show];
        } else if (WIDTH==320  && my_zhuZe_qianMing.length<2200) {
            ...
            [alertView show];
        } else if (WIDTH==375  && my_zhuZe_qianMing.length<3000) {
            
            UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"请正确填写“主责评估员签名”" message:nil delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
            [alertView show];
        } else if (WIDTH==414  && my_zhuZe_qianMing.length<4000) {
