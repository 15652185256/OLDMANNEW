...
-(void)update_SheHuiCY
{
    ...
    //查询 社会参与
    /*
    if ([[FMDBManager shareManager]selectSheHuiCYByShenFenZJ:self.shenFenZJ].count>0) {
        NSLog(@"存在！");
        
        //修改 社会参与
        [[FMDBManager shareManager]updateSheHuiCYByShenFenZJ:self.shenFenZJ dict:parameter];
    } else {
        NSLog(@"不存在！");
        
        //添加 社会参与
        [[FMDBManager shareManager]addSheHuiCYData:parameter];
    }
    
    
    //删除 社会参与
    //[[FMDBManager shareManager]deleteSheHuiCYByShenFenZJ:self.shenFenZJ];
     */
